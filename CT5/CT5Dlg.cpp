...
BOOL CCT5Dlg::LoadFile(LPTSTR lpFilename)
{
	hFile = CreateFile(lpFilename, GENERIC_READ, \
		...
		OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);//몽앴匡숭츰댔역匡숭,돤돕匡숭얌깨.
	if (!hFile)
		return FALSE;
	hFileMapping = CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, 0, NULL);//눼쉔鄲獗匡숭
	...
	ImageBase = MapViewOfFile(hFileMapping, FILE_MAP_READ, 0, 0, 0);//돤돕鄲獗匡숭돨샘囹,닸렴瞳홍애긴좆ImageBase櫓.
	if (!ImageBase)
	{
		CloseHandle(hFile);
		return FALSE;
